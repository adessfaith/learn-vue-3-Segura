<template>
  <h1>{{ message }}</h1>
  <h1>Welcome, {{ formData.name }}</h1> /<!--really is a lot quicker than DOM binding-->
  <form action="">
    <div>
      <span>Name:</span>
      <span>
    ...
    <div>
      ...
      <span>
        ...
        <input type="text" id="age" v-model.number="formData.age" /> <!--v-model modifers-->
      </span>
    </div>
    <div>
      <span>Hobbies:</span>
      <!-- We can bind multiple checkboxes to the same array. -->
      <input type="checkbox" value="basketball" v-model="formData.hobbies" /> <!--bind the value of the checkbox to an array-->
